  ...
  _createClass(WordCloud, [{
    ...
    value: function render() {
      ...
      var layout = (0, _d3Cloud2.default)().size([width, height]).font(font).words(wordCounts).padding(padding).rotate(rotate).fontSize(fontSizeMapper).on('end', function (words) {
        d3.select(_this2.wordCloud).append('svg').attr('width', layout.size()[0]).attr('height', layout.size()[1]).attr('class', className).append('g').filter(function (d) {
          return d.size < minSize;
        }).attr('transform', 'translate(' + layout.size()[0] / 2 + ',' + layout.size()[1] / 2 + ')').selectAll('text').data(words).enter().append('text').style('font-size', function (d) {
          return Math.min(Math.max(d.size, minSize), maxSize) + 'px';
